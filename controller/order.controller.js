...
export const selectUserBillingAddressController = async (req, res) => {
    try {
        ...
        if (!billingaddressId || !mongoose.Types.ObjectId.isValid(billingaddressId)) {
            return sendBadRequestResponse(res, "Invalid or missing billingaddressId");
        }
        ...
        if (user.selectedBillingAddress?.toString() === billingaddressId) {
            return sendSuccessResponse(res, "BillingAddress already selected", {
                selectedBillingAddress: user.selectedBillingAddress
            });
        }

        // Update selected billingaddress
        user.selectedBillingAddress = billingaddressId;
        await user.save();

        return sendSuccessResponse(res, "BillingAddress selected successfully", {
            selectedBillingAddress: user.selectedBillingAddress
        });

    } catch (error) {
        console.error("Error while selecting billingaddress:", error.message);
        return sendErrorResponse(res, 500, "Error while selecting billingaddress", error);
    }
};
